...
### Combines what used to be main_analysis_preprocessing.R and main_analysis.R into a single, cleaner script, ommited steps and more detailed descriptions of choices can be found there. Seriously. Read the other one to rationalise some choices and look at some explanations!
### IGR 08.18.14
...
outdir='output/PostAlignment/'
#######################################################
### 1. READ IN GENES AND GENERATE TABLES AND META FILES 
#######################################################

chimp.counts <- fread(paste('./pantro5/',outdir,"ipsc_chimp_counts.txt",sep=''), header=T)
human.counts <- fread(paste('./hg38/',outdir,"ipsc_human_counts.txt",sep=''), header=T)

human.counts <- read.table("ipsc_human_ensembl89_0.97_ortho_FALSE_clean.out", header=T, stringsAsFactors=T)
ipsc.counts <- merge(chimp.counts[,c(1,3:10)], human.counts[,c(1,3:10)], by.x="Geneid", by.y="Geneid", all=T)
names(ipsc.counts)[1] <- "EnsemblID"
dim(ipsc.counts)
# [1] 40075    17

exon.lengths = 'orthoexon_files/metaOrthoExon_hg38_panTro5_ensembl86_0.97_ortho_FALSE_pc.txt'

exon.lengths <- read.table("~/reference/hg19v89_panTro3_ortho/metaOrthoExon_hg19_panTro3_ensembl89_0.97_ortho_FALSE_unstranded_pc.txt", header=T)
gene.lengths <- ddply(exon.lengths, "gene", function(x) colSums(x[,c(6,11)]))
gene.lengths$diff <- abs(gene.lengths$widthHs - gene.lengths$widthpanTro3)

#remove unwanted individiuals.
...
row.names(ipsc.counts) <- ipsc.counts[,1] ### this may seem dumb, but it makes downstream comparisons a lot easier - certainly much more so than pulling things out of ipsc.genes.dge$genes....
dim(ipsc.counts)
# [1] 40071    15 # What are these genes?
...
ipsc.counts <- ipsc.counts[!ipsc.counts$EnsemblID %in% genes.to.discard$x,]
dim(ipsc.counts)
# [1] 32893    15

#prepare meta information and plot parameters:
ipsc.counts.names <- colnames(ipsc.counts[,2:15])
rpkm.col <- c(rep("darkorchid4", 7), rep("black", 7))
...
samples.meta$cex <- 1.5
    ...
    samples.meta$species <- c(rep("chimp", 7), rep("human", 7))
    samples.meta$pch.species <- c(rep(16,7), rep(15,7))
    samples.meta$pch.method <- c(rep(16,7), 15, 14, 15, 15, 17, 17, 14)
    samples.meta$col.sex <- c("plum4", "yellow3", "yellow3", "yellow3", "plum4", "yellow3", "plum4", "yellow3", "yellow3", "plum4", "yellow3", "yellow3", "plum4", "plum4")
	...
	row.names(samples.meta) <- samples.meta$line
...
ipsc.genes.dge <- DGEList(counts=as.matrix(ipsc.counts[,2:13]), genes=as.character(ipsc.counts[,1]))
...
## "Raw" library sizes:
pdf(file="plots/mapped_reads_raw_ipsc_final_no_ribo.pdf")
...
pdf(file="plots/mapped_reads_normalised_ipsc_final_no_ribo.pdf")
...
pdf(file="plots/number_of_genes_expressed_ipsc_final_no_ribo.pdf")
...
pdf(file="plots/rarefaction_curves_ipsc_final_no_ribo.pdf")
...
pdf("cpm.density_ipsc_final_no_ribo.pdf")
...
# [1] 12674    12
pdf("cpm.filtered.density_ipsc_final_no_ribo.pdf")
...
pdf(file="voom.loess.norm.norandom.density_ipsc_final_no_ribo.pdf")
...
pdf(file="voom.quantile.norm.norandom.density_ipsc_final_no_ribo.pdf")
...
rpkm.loess.norm.norandom.voom$E[,1:6] <- rpkm.loess.norm.norandom.voom$E[,1:6] - log2(gene.lengths[gene.lengths$gene %in% row.names(rpkm.loess.norm.norandom.voom),3]/1000)
rpkm.loess.norm.norandom.voom$E[,7:12] <- rpkm.loess.norm.norandom.voom$E[,7:12] - log2(gene.lengths[gene.lengths$gene %in% row.names(rpkm.loess.norm.norandom.voom),2]/1000)
pdf(file="rpkm.voom.loess.norm.density_ipsc_final_no_ribo.pdf")
...
pdf(file="plots/pc1_vs_pc2_loess_norandom_voom_ipsc_final_no_ribo.pdf")
plot(ipsc.norm.rpkm.pca$x[,1], ipsc.norm.rpkm.pca$x[,2], pch=samples.meta$pch.species, col=as.character(samples.meta$col.method), cex=1.5, xlab=paste("PC1 (", round(pca.var[1]*100, digits=2), "% of variance)", sep=""), ylab=paste("PC2 (", round(pca.var[2]*100, digits=2), "% of variance)", sep=""), main=paste("Loess GC norm RPKM at gene level (n=", dim(rpkm.loess.norm.norandom.voom$E)[1], ")", sep=""))
legend(x="topleft", col=c("plum4", "yellow3", "firebrick4", "lightpink"), pch=c(16,15,15,15), pt.cex=1.5, legend=c("Chimpanzee (fib)", "Yoruba (LCL)", "Caucasian (LCL)", "Caucasian (fib)"), bty="n", cex=0.8)
dev.off()    
...
# 1   1 2.546739e-12 0.008955436       1.116076e-10 2.884740e-11
# 2   2 9.465002e-01 0.846787083       9.444060e-01 6.544503e-03
# 3   3 9.936855e-01 0.058720162       1.264652e-02 6.467735e-01
# 4   4 8.457223e-01 0.926015182       9.803947e-01 7.406730e-01
# 5   5 9.680787e-01 0.304806159       3.237262e-01 7.743050e-01
# 6   6 9.542689e-01 0.598505477       7.968170e-01 9.926726e-01
# 7   7 9.909433e-01 0.999153758       9.998931e-01 7.146764e-01
# 8   8 9.643626e-01 0.808867968       9.304552e-01 9.724498e-01
# 9   9 9.296357e-01 0.614294785       7.303065e-01 8.366655e-01
# 10 10 9.794283e-01 0.918365651       9.929924e-01 9.994384e-01
# 11 11 9.594768e-01 0.841867461       9.475391e-01 9.964101e-01
# 12 12 7.922911e-02 0.259282320       2.324219e-01 4.780593e-02
...
pdf(file="plots/model_MA_plots_design_loess.norm.norandom.voom_ipsc_final_no_ribo.pdf")
    par(mfrow=c(1,2))
...
pdf(file="plots/model_MA_plots_design.sex.interact_loess.norm.norandom.voom_ipsc_final_no_ribo.pdf")
    par(mfrow=c(2,2))
...
pdf(file="plots/model_SA_plots_loess.norm.norandom.voom_ipsc_final_no_ribo.pdf")
    par(mfrow=c(1,2))
...
pdf(file="plots/model_residual_boxplots_loess.norm.norandom.voom_ipsc_final_no_ribo.pdf")
...
# Non GC data:
write.table(topSpecies.loess.norm.norandom, file="topSpecies.loess.norm.norandom_ipsc_final_no_ribo.out", row.names=T, col.names=T, quote=F)
write.table(topSpecies.loess.norm.norandom.sex.interact, file="topSpecies.loess.norm.norandom.sex.interact_ipsc_final_no_ribo.out", row.names=T, col.names=T, quote=F)
write.table(topSpecies.loess.norm.norandom.sex.fm, file="topSpecies.loess.norm.norandom.sex.fm_ipsc_final_no_ribo.out", row.names=T, col.names=T, quote=F)
