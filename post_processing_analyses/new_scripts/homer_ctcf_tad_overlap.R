...
# all_tads <- rbind(common_tads_hg38,ssp_tads)
# all_tads <- all_tads[,tadID:=paste('tad_',seq(1:nrow(all_tads)),sep='')]
...
# upstream_boundaries = copy(boundaries)%>%
# dplyr::select(c('seqnames',contains('upstream'),'species','tadID'))%>%
# setnames(old=c('upstream_start','upstream_end'),new=range_keys[-1])%>%
# mutate('boundary'='upstream')

# downstream_boundaries = copy(boundaries)%>%
# dplyr::select(c('seqnames',contains('downstream'),'species','tadID'))%>%
# setnames(old=c('downstream_start','downstream_end'),new=range_keys[c(2,3)])%>%
# mutate('boundary'='downstream')
...
# tadbound_overlap <-function(peaks,tadbound){
#     tads <-copy(tadbound)%>%split(by='tadID')
#     overlap <- lapply(tads,function(x){
#         x<-foverlaps(peaks,x,type='any')%>%na.omit()
#         x <- x[
#             ,distance_ctcf_tad:= ifelse(boundary=='upstream',i.end-end,i.start-start)
#             ][
#                 ,c('start','end','species','boundary'):=NULL
#                     ]%>%setnames(old=c('i.start','i.end'),new=c(range_keys[-1]))
#     })
#     overlap <- Map(mutate,overlap,tadID=names(overlap))%>%rbindlist()
#     return(overlap)
# }
...
## for all other peaks
nonctct_tadbound_overlap <- lapply(tad_boundaries,function(x){
    nonctcf_peaks<- copy(da_results)[!peakID %in% ctcf_motifs$peakID]
    tad_overlap <- tadbound_overlap(nonctcf_peaks,x)
    }
)
