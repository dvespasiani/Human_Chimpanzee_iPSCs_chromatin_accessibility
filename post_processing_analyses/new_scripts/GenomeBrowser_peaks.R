...
cmean <- rowMeans(test[,c(1:6)])
hmean <- rowMeans(test[,c(7:12)])
...
top_differences <- copy(diff_means)[abs(c_h_mean_diff)>10]%>%setorderv('c_h_mean_diff',1)

top_different_peaks <- list('peak_101698','peak_196193')
## peak_196193 more accessible in human. target gene involved in development of human germ cells
## peak_101698 more accessible in chimp. target gene is expressed in developing (but not adult) brain
...
peak_of_interest <-copy(peak_gene_of_interest)%>%lapply(function(x){x<-x[,c(..range_keys)]})

## get region of interest
tad_of_interest <- copy(peak_gene_of_interest)%>%lapply(
    function(x){
        x<- x[
            ,start:=tad_start
            ][
...
chromstate_tad_of_interest <- copy(tad_of_interest)%>%lapply(
    function(x){
        setkeyv(x,range_keys)
...
expanded_region <- copy(tad_of_interest)%>%lapply(function(x)
    x<-x[
...
biomart_track <- copy(tad_of_interest)%>%lapply(function(x){
...
biomart_track <-purrr::map2(biomart_track,peak_gene_of_interest,function(x,y){
    x<-x[x@range$symbol %in% y$gene]
    return(x)
    }
)
...
## chromstate track
ipsc_chromstate_track <- lapply(chromstate_tad_of_interest,function(x){
    track <- DataTrack(
            range = x,
            genome = genome,
...
chimp_tracks <- c(gtrack[[1]],tad_track[[1]],conservation_track[[1]],ipsc_chromstate_track[[1]],biomart_track[[1]],ht[[1]])
human_tracks <- c(gtrack[[2]],tad_track[[2]],conservation_track[[2]],ipsc_chromstate_track[[2]],biomart_track[[2]],ht[[2]])

## plot tracks
pdf(paste0(plot_dir,'chimp_da_genome_plot.pdf',sep=''),width = 10, height = 10)
plotTracks(
    chimp_tracks,
...
pdf(paste0(plot_dir,'human_da_genome_plot.pdf',sep=''),width = 10, height = 10)
plotTracks(
    human_tracks,
