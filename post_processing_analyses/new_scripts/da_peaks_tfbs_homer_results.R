...
## read motifs
##-------------
homer_known_files <- list.files(homer_dir,recursive=T,full.names=T,pattern='knownResults.txt')

homer_known_results <- lapply(homer_known_files,function(x){ 
  known_motifs <-fread(x,sep='\t',header=T)[,motif_name:=gsub("\\/.*","",`Motif Name`)]
  motif_family <- stringr::str_extract(string = known_motifs$motif_name,pattern = "(?<=\\().*(?=\\))")
...
names(homer_known_results) = species_names

homer_known_results <- Map(mutate,known_motif,species=species_names)

number_initial_motifs = copy(homer_known_results)%>%lapply(function(x)as.data.table(x)[,'motif_name']%>%unique()%>%nrow())
##----------------
## filter motifs
##----------------
## FDR adjust pvalues
