...
peak_scores <- Map(mutate,peak_scores,peak_species = names(peak_scores))%>%rbindlist()
...
lapply(
    ...
    avg_score_quantile~peak_species,
    x,
    method='wilcox.test', 
    ref.group='common'
    )
...
test_stat <- test_stat[,c('group2','quantile_range','p.adj','p.signif')][,quantile_range:=as.numeric(quantile_range)]%>%unique()%>%setnames(old=1,new='peak_species')
mock_stat <- copy(test_stat)[,peak_species:='common'][,p.adj:=0][,p.signif:=' ']
...
ggplot(peak_scores, aes(x = as.factor(quantile_range), y=avg_score_quantile,fill=peak_species))+
