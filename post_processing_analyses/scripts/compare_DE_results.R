...
## my DE results 
my_results <- fread(
    paste(de_dir,"topSpecies.loess.norm.norandom_ipsc_final_no_ribo.out",sep=''),sep=' ',header=F,
    col.names = c('genes','EnsemblID','logFC','AveExpr','t','P.Value','adj.P.Val','B')
    )[
        ,significant:=ifelse(adj.P.Val<=pval_threshold,'s','ns')
]
## Irene's
igr_results <-  fread(paste(de_dir,"gallego_romero_etal_2015_de_genes.txt",sep=''),sep='\t',header=T)[,significant:=ifelse(adj.P.Val<=pval_threshold,'s','ns')]
...
length(unique(common_de_results$EnsemblID))/length(unique(igr_de$EnsemblID))*100
# 68.0%
